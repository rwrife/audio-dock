...
The required manual-scenario labels are defined once, by `Get-RequiredManualScenario` in `scripts/collect-evidence.ps1`; `finalize-evidence.ps1` validates against that same list, and `docs/windows-release-evidence-template.md` mirrors it verbatim. `tests/powershell/test-evidence-template-sync.ps1`, executed by CI on every pull request, fails if the template ever drifts from the canonical labels, so an operator following the template can never be told the gate is missing scenarios the template never showed.

After an operator completes the manual scenario table in a collected bundle, `.\scripts\finalize-evidence.ps1` validates the bundle's bookkeeping: every required scenario row present with an explicit pass/fail verdict and evidence reference, honesty notes filled, ZIP checksum matching, and no failed or errored automated commands. It reports per-row blockers and exits non-zero until the bundle is complete and consistent. It verifies documentation completeness only — it cannot observe physical behavior, and a completed bundle still needs human review before any release decision. Its logic is covered by `tests/powershell/test-finalize-evidence.ps1`, also executed by CI on every pull request.
