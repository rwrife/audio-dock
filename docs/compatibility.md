# Read-only Core Audio compatibility and evidence

## Scope

Milestone 2 inventories Core Audio metadata only. It never opens an audio stream and has no API or command for changing endpoint roles, volumes, mute state, or sessions. Endpoint-role mutation is intentionally not implemented.
...
Each of the console, multimedia, and communications defaults is queried independently with `GetDefaultAudioEndpoint` for render and capture. The returned endpoint ID is compared ordinally with enumerated IDs. Missing defaults are left unassigned. This is observation only: no undocumented policy-configuration interface is used and no role is changed.
...
There is no mutation fallback in this milestone. Consumers may show the diagnostic or guide the user to Windows Settings, but must not claim Audio Dock changed a role or value.
